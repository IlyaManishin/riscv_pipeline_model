...
| `br_comparator.py` | `BranchUnit.compare(rd1, rd2, br_un)` | Returns `(br_eq, br_lt)`. Signed compare when `br_un=False`, unsigned when `br_un=True`. |
...
- The **single-cycle** core calls `Instruction_Decoder`, `ImmGen`,
  `BranchComparator`, `Alu`, `Shifter`, and the `dmem_*_port` helpers inside
